...
const Diarydetails = dynamic(() => import('@/components/diarydetails/Diarydetails'), {
    loading: () => <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
            <div className="bg-blue-100 p-4 rounded-lg shadow-md inline-block">
                <p className="text-[#044093] text-lg font-medium">
...
async function page({ params, searchParams }) {
    ...
    const { diaryuid, diarypageid } = params;
    const cookieStore = await cookies();
    const uuid = cookieStore.get('uuid')?.value;
    const is_logged = cookieStore.get('is_logged')?.value;
    let sessionToken = cookieStore.get('unregistered_user_token')?.value || "null";

    // if (is_logged === "false" && !sessionToken) {
    //     sessionToken = randomUUID();
    // }

    const getDiarypageContent = await getDiaryPageContent(diaryuid, diarypageid, uuid, sessionToken);
    ...
    return (
        <Suspense>
            ...
            <Headwrapper
                uuid={uuid}
            />
            <AuthCheck uuid={uuid} is_logged={is_logged} href={`/diarydetails/${diaryuid}/${diarypageid}`}>
                <Diarydetails
                    diaryuid={diaryuid}
                    user_uid={uuid}
                    is_logged={is_logged}
                    diarypageno={diarypageid}
...
async function getDiaryPageContent(diaryuid, diarypageid, uuid, sessionToken) {
    try {
...
export async function generateMetadata({ params }) {
    try {
        ...
        // const imageUrl = extractFirstImage(diary?.content) || diary?.featured_image_url;
