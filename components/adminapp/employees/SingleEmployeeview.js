...
import author_1 from '@/public/assets/author_1.svg';
...
function SingleEmployeeview({ closeSingleuserview, singleUserid, openEditEmployeemodal, setRefreshStatus, refreshStatus }) {
    ...
    async function getSingleEmployee(singleUserid) {
        Employeeapi.get('/getSingleEmployeeData', {
            params: {
    ...
    useEffect(() => {
        ...
        getSingleEmployee(singleUserid);
    ...
    useEffect(() => {
        if (refreshStatus) {
            setIsLoadingEffect(true);
            getSingleEmployee(single_user_id);
    ...
    return (
        <div className=' flex flex-col w-full'>
            <div className="flex flex-row justify-between items-center px-4 py-4 border-b-[0.4px]">
                <div className='flex flex-row items-center gap-2'>
                    <Image src={author_1} alt="author_1" width={50} height={50} className='rounded-full' />
                    <div>
                        <p className='max-sm:text-[14px] md:text-[16px] text-[#2B2B2B]'>{userdata.name} </p>
                        <p className=' max-sm:text-[10px] text-sm text-[#676767]'>{userdata.role_name}</p>
                    </div>
                </div>
                <div className="flex flex-row gap-x-3">
                    <button
                        onClick={() => openEditEmployeemodal(userdata.id, userdata.role_name)}
                        className="flex justify-center items-center relative gap-2.5 px-5 py-1.5 rounded-sm bg-[#044093]"
                    >
                        <p className="flex-grow-0 flex-shrink-0 text-sm text-left text-white cursor-pointer">
                            Edit
                        </p>
                    </button>
                    <button onClick={closeSingleuserview} className='cursor-pointer'>
                        <svg
                            width={18}
                            height={18}
                            viewBox="0 0 18 18"
            ...
            <div className="flex justify-start items-center gap-12 mt-3.5 px-[20px]">
                <div
                ...
                <div className='relative'>
                    <div className="flex flex-col justify-start items-start gap-4 p-5">
                        <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative gap-4">
                            ...
                            <p className="flex justify-start  relative gap-1.5 flex-grow-0 flex-shrink-0 text-sm text-left text-[#2b2b2b]">
                                {userdata.name}
                            </p>
                        </div>
                        <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative gap-4">
                            ...
                            <p className="flex justify-start flex-grow-0 flex-shrink-0 relative gap-1.5 text-sm text-left text-[#2b2b2b]">
                                {userdata.email}
                            </p>
                        </div>
                        <div className="flex justify-start self-stretch flex-grow-0 flex-shrink-0 relative gap-4">
                            <p className="flex-grow-0 flex-shrink-0 w-[150px] text-sm text-left text-[#2b2b2b]">
                                Phone Number
                            </p>
                            {userdata.phone === null || userdata.phone === '' || userdata.phone === undefined ?
                        ...
                        <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative gap-4">
                            ...
                            {userdata.role_name === null || userdata.role_name === '' || userdata.role_name === undefined ?
                                <p className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-1.5 text-sm text-left text-[#2b2b2b]">
                                    ---
                                </p>
                                :
            ...
            {
                activeProfileTab === 'changepassword' && (
                    <Changepassword singleUserid={singleUserid} />
                )
            }
            {
                errorMessage !== '' &&
