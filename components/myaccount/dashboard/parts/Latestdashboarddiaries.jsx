...
function Latestdashboarddiaries({ latestDiriesLoading, latestDiries }) {
    return (
        <React.Fragment>
            <div className='shadow-[0px_0px_3px_rgba(0,0,0,0.1)] bg-white rounded-[5px] p-2 mt-2'>
                <p className='text-[14px] 2xl:text-[16px] 4xl:text-[18px] font-semibold p-2'>Latest Diaries</p>
                <div className="w-full relative overflow-x-auto rounded-[4px]">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {
                                latestDiriesLoading === false ?
                                    latestDiries.length > 0 ?
                                        latestDiries.map((diary, index) => (
                                            <tr key={index} className="truncate border-b-[0.6px] border-b-[#979797]/40 ">
                                                ...
                                                <td className='text-center sticky_column_last'>
                                                    <Link href={`/myaccount/diary/${diary?.name}?uid=${diary?.uuid}`} className='flex items-center justify-center'>
...
export default Latestdashboarddiaries